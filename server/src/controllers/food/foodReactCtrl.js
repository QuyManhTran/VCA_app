const { source } = require("../../configs/cloundinary.config");
...
const likeReact = async (req, res) => {
  ...
  for (let i = 0; i < like_list.length; i++) {
    if (food_id == like_list[i].items) {
      ...
      await Food.findOneAndUpdate({ _id: food_id }, { like: food.like - 1 });
      ...
      return res.status(200).json({ like: food.like - 1 });
    }
  }
  await Food.findOneAndUpdate({ _id: food_id }, { like: food.like + 1 });
...
const rateReact = async (req, res) => {
  ...
  for (let i = 0; i < food.rate_list.length; i++) {
    if (food.rate_list[i].user_id === user_id) {
      ...
      console.log(newRate);
      // update rate food
      await Food.findOneAndUpdate({ _id: food_id }, { rate: newRate });
      return res.status(200).json({ rate: newRate });
    }
  }
