...
const createNewList = async (req, res, next) => {
  ...
  try {
    await newUList.save().then((ulist) => {
      res.status(200).json({
        id: ulist._id,
        name: ulist.name,
        success: true,
...
const getAllList = async (req, res, next) => {
  ...
  try {
    ...
    const newAllList = allList.map((list, index) => {
      return { id: list._id, name: list.name, listFood: list.listFood };
    });
...
const getSingleList = async (req, res, next) => {
  ...
  ulist
    .find({ _id: id_ulist, id_user: id_user })
    .then((list) => {
...
const addItemToMutilList = async (req, res, next) => {
  ...
  try {
    const ulistDetails = await ulist.find({
...
const deleteMultiItemList = async (req, res, next) => {
  const { id_user, id_ulist, id_foods } = req.body;
  console.log(id_user, id_ulist, id_foods);
  try {
    await ulist.updateOne(
      { _id: id_ulist, id_user: id_user },
      { $pull: { listFood: { $in: id_foods } } }
    );
